...
qt_branch=dev                   # Qt version to use, use latest development version
...
curl -L -s https://github.com/openssl/openssl/archive/${openssl_ver}.tar.gz | tar xz
[[ $? -eq 0 ]] || exit 1

cd openssl-${openssl_ver}

./config zlib --prefix=${depsdir} -mmacosx-version-min=${min_macos_ver}
[[ $? -eq 0 ]] || exit 1
make -j4 || exit 1
...
curl -L -s "https://www.dropbox.com/s/0kp0m0hy9lkxdql/qtbase-dev.patch?dl=1" | patch -p1 -d qtbase
curl -L -s "https://www.dropbox.com/s/8obu07g04l17m7f/qttools-dev.patch?dl=1" | patch -p1 -d qttools
curl -L -s "https://bugreports.qt.io/secure/attachment/79380/pb-fix.patch" | patch -p1

qtbuilddir="../build-qt"
mkdir ${qtbuilddir} && cd ${qtbuilddir}
# all -skip instructions are completely optional here, and nothing will changed if they are not specified
# in case of downloading from git repo using my own method, but I left them here for better compatibility
${workdir}/qt5/configure -prefix "${depsdir}/Qt" -opensource -confirm-license -release -appstore-compliant -c++std c++14 -no-pch -I "${depsdir}/include" -L "${depsdir}/lib" -skip qt3d -skip qtactiveqt -skip qtandroidextras -skip qtcanvas3d -skip qtcharts -skip qtconnectivity -skip qtdatavis3d -skip qtdeclarative -skip qtdoc -skip qtgamepad -skip qtgraphicaleffects -skip qtimageformats -skip qtlocation -skip qtmultimedia -skip qtnetworkauth -skip qtpurchasing -skip qtquickcontrols -skip qtquickcontrols2 -skip qtremoteobjects -skip qtscript -skip qtscxml -skip qtsensors -skip qtserialbus -skip qtserialport -skip qtspeech -skip qtvirtualkeyboard -skip qtwayland -skip qtwebchannel -skip qtwebengine -skip qtwebglplugin -skip qtwebsockets -skip qtwebview -skip qtwinextras -skip qtx11extras -skip qtxmlpatterns -make libs -no-compile-examples -no-dbus -no-icu -qt-pcre -system-zlib -ssl -openssl-linked -no-cups -qt-libpng -qt-libjpeg -no-feature-testlib -no-feature-sql -no-feature-concurrent
[[ $? -eq 0 ]] || exit 1
make -j4 || exit 1
make install || exit 1
...
boost_ver=1.69.0                # Boost version to use

boost_ver_u=${boost_ver//./_}
curl -L -s https://dl.bintray.com/boostorg/release/${boost_ver}/source/boost_${boost_ver_u}.tar.bz2 | tar xj
[[ $? -eq 0 ]] || exit 1
...
then
  ...
  curl -L -s https://cmake.org/files/v3.14/cmake-3.14.0-Darwin-x86_64.tar.gz | tar xz
  cmakedir=$(ls | grep cmake)
  cmake="${workdir}/${cmakedir}/CMake.app/Contents/bin/cmake"
fi
[[ -f "${cmake}" ]] || exit 1

lt_branch=RC_1_2                # libtorrent version to use, use latest development version from 1.1.x versions

curl -L -s https://github.com/arvidn/libtorrent/archive/${lt_branch}.tar.gz | tar xz
[[ $? -eq 0 ]] || exit 1
...
export CXXFLAGS="-std=c++14 -mmacosx-version-min=${min_macos_ver}"
${cmake} -DCMAKE_PREFIX_PATH=${depsdir} -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX=${depsdir} ..
...
# next part os this script is part from my another script used to build my own projects for macOS.
